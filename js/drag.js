...
Drag.prototype = { //初始化
	initialize: function (drag, options) {
		...
		this.isPC = DeviceType();
		this.addHandler(this.handle, (this.isPC ? "mousedown" : "touchstart"), this.bind(this, this.startDrag))
	},
	...
	startDrag: function (event) {
		...
		this._x = (this.isPC ? event.clientX : event.touches[0].clientX) - this.drag.offsetLeft;
		this._y = (this.isPC ? event.clientY : event.touches[0].clientY) - this.drag.offsetTop;
	...
	moveDrag: function (event) {
		var event = event || window.event;
		var iTop = (this.isPC ? event.clientY : event.touches[0].clientY) - this._y + getScrollTop() - this._h;
		var iLeft = (this.isPC ? event.clientX : event.touches[0].clientX) - this._x;
	...
	addHandler: function (oElement, sEventType, fnHandler) { // 添加绑定事件
		return oElement.addEventListener ? oElement.addEventListener(sEventType, fnHandler, false) : oElement.attachEvent("on" + sEventType, fnHandler)
	},
	removeHandler: function (oElement, sEventType, fnHandler) { // 删除绑定事件
		return oElement.removeEventListener ? oElement.removeEventListener(sEventType, fnHandler, false) : oElement.detachEvent("on" + sEventType, fnHandler)
	},
	bind: function (object, fnHandler) { // 绑定事件到对象
		return function () {
